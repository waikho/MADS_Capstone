...
def getTicker(symbol, config=config.pgSecrets):
    #Init
...
def getTickerList(config=config.pgSecrets):
    pgConnStr = pgDictToConn(config)
    with psycopg.connect(pgConnStr) as conn:
        with conn.cursor(row_factory=dict_row) as cur:
...
def getFilteredTickerList(lowest_price=0.0, highest_price=999999999,config=config.pgSecrets):
    pgConnStr = pgDictToConn(config)
    with psycopg.connect(pgConnStr) as conn:
        with conn.cursor(row_factory=dict_row) as cur:
